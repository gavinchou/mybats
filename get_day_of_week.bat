...
::set str="%m:~0,1%"
::echo %str%
::if %str%=="0" set m=%m:~1,1%
::set str="%d:~0,1%"
::if %str%=="0" set d=%d:~1,1%
...
REM if %y% gtr 0 (set /a year=2000+%y:~-2,2%%%28) else goto error
set /a yu=%y%%%4
set /a yu=%y%%%4
...
set /a ml=%m%-1
set /a long=0
REM if %m% leq 7 (set /a long=%ml%*31-%ml%/2+%d%) else (set /a long=%ml%*30+%ml%/2+%d%)
